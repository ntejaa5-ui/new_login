...
// In a real project, this would be imported from ./RoleSelection.jsx
const RoleSelection = () => {
  ...
  // Navigate to login instead of just back
  ...
  return (
    <form onSubmit={handleSubmit}>
      <ArrowField label="Full Name" icon={User}>
        <input type="text" placeholder="e.g. Jane Doe" className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition" required />
      </ArrowField>
      <ArrowField label="Designation" icon={Briefcase}>
        <input type="text" value={designation} onChange={(e) => setDesignation(e.target.value)} placeholder="e.g. Senior Software Engineer" className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition" required />
      </ArrowField>
      <ArrowField label="Experience" icon={Clock}>
        <div className="flex items-center gap-2">
          <input type="number" placeholder="5" className="w-24 px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition" required />
      ...
      <ArrowField label="Technologies" icon={Code}>
        <div className="space-y-3">
          <div className="flex gap-2">
            <input type="text" value={currentTech} onChange={(e) => setCurrentTech(e.target.value)} placeholder="Add tech" className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition" onKeyPress={(e) => e.key === 'Enter' && addTech(e)} />
            <button onClick={addTech} type="button" className="bg-gray-100 hover:bg-gray-200 text-gray-700 p-2 rounded-lg transition"><Plus size={20} /></button>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {techList.map((tech) => (
              <span key={tech} className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                {tech}
                <button type="button" onClick={() => removeTech(tech)} className="hover:text-blue-900"><X size={14} /></button>
              </span>
            ))}
          </div>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <ArrowField label="Your Name" icon={User}>
        <input type="text" placeholder="e.g. John Smith" className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 outline-none transition" required />
      </ArrowField>
      <ArrowField label="Current Role" icon={Briefcase}>
        <input type="text" value={currentRole} onChange={(e) => setCurrentRole(e.target.value)} placeholder="e.g. Student" className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 outline-none transition" required />
      </ArrowField>
      <ArrowField label="To Learn" icon={BookOpen}>
        <div className="space-y-3">
          <div className="flex gap-2">
            <input type="text" value={currentInterest} onChange={(e) => setCurrentInterest(e.target.value)} placeholder="What do you want to learn?" className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 outline-none transition" onKeyPress={(e) => e.key === 'Enter' && addInterest(e)} />
            <button onClick={addInterest} type="button" className="bg-gray-100 hover:bg-gray-200 text-gray-700 p-2 rounded-lg transition"><Plus size={20} /></button>
          </div>
          <div className="flex justify-end">
          ...
          <div className="flex flex-wrap gap-2">
            {interestList.map((item) => (
              <span key={item} className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                {item}
                <button type="button" onClick={() => removeInterest(item)} className="hover:text-green-900"><X size={14} /></button>
              </span>
            ))}
          </div>
        </div>
...
// --- COMPONENTS SECTION (HOME DASHBOARD) ---
